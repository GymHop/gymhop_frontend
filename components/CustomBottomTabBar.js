  ...
  render() {
    ...
    return (
      <SafeAreaView style={tabBarStyle} forceInset={safeAreaInset}>
        {routes.filter((route) => {
          // filters out the gymowner tab entirely
          // if the user isnt authenticated
          if (this.props.isGymOwner) {
            return true
          } else {
            if (route.routeName == "gymOwner") {
              return false
            }
            return true
          }
...
function mapStateToProps(state) {
  return {
    isGymOwner: state.user.details.is_gym_owner
  }
}

export default withDimensions(connect(mapStateToProps)(TabBarBottom));
